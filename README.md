...
- **‼️ KB-update gate on Review/Done.** A ticket may NOT be moved to **Review** or **Done** until its knowledgebase impact has been recorded as a comment on that ticket. If the work changed or added anything a future reader would want in the KB (architecture, conventions, runbook/ops, gotchas, the project overview's light notes, future considerations, command references, etc.), update the relevant KB doc(s) FIRST, then comment on the ticket citing each updated article's key (e.g. "KB: updated LDA-KB-12"). If nothing in the KB needs to change, comment that explicitly (e.g. "KB: no change needed"). Either way the decision lives in the ticket history — the goal is that every scrap of work is traceable through ticket comments AND the KB stays fully current. This gate is per-ticket judgement ("where applicable"), but recording the judgement is mandatory.
...
- **Every project KB has one canonical overview doc, titled EXACTLY `Project overview & KB index`.** It's the lightweight landing page: a one-line "what this project is", an index table mapping every KB doc to a one-line description, and a "Light notes" section for passing notes that don't yet warrant their own article. If a project doesn't have this doc yet, create it (the fixed title means `ldash_save_kb_doc` upserts the same doc on every update). Keep it current — when you add/rename/retire a KB doc or record a deferred decision, reflect it here. It must always link to the project's snapshot doc (the second anchor — see the reading rule below). Read this doc first to orient on a project.
- **Two anchor docs + the reading rule (entry-point → auto-read snapshot → lazy-read the rest).** Every project KB has TWO anchor docs: the `Project overview & KB index` (the entry point, stable map) and a **current-state snapshot doc** (the moving picture — what's shipped, in flight, queued; per-project title, e.g. `Current state snapshot` or `Handover`). The rule when you read the overview: **AUTOMATICALLY also read the snapshot doc it links to** — the two are meant to be read together to orient. Do **NOT** automatically read the other articles indexed in the overview; pull those **on demand only**, so a cold start loads just the two anchors and doesn't bloat the context window. The overview must always carry an explicit link to its snapshot so the auto-read target is unambiguous; if a project has no snapshot doc yet, create one and link it.
- **The LOCAL repo CLAUDE.md hard-codes the lookup coordinates** so a fresh agent skips the ldash discovery round-trip. The checked-in `CLAUDE.md` loads automatically at session start, so its ldash session-start block should record (a) the **ldash project id** (no `ldash_list_projects` → name-match needed) and (b) the **keys/ids of the two anchor docs** — the overview and the snapshot — so the agent can `ldash_get_kb_doc` them directly. Net effect on cold start: read the already-loaded local CLAUDE.md, fetch exactly the overview + snapshot by id with zero discovery calls, then lazy-load the rest. Keep these coordinates current: whenever the overview or snapshot key changes (or the snapshot is renamed), update the local CLAUDE.md coordinates to match.
