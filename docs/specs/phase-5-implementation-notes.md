...
### 13. Planning prompt — atomicity-based task convention (2026-06-10)

The ARTIFACT CONVENTIONS section of `server/src/planning/prompt.ts` was rewritten to define tasks by atomicity rather than time. Key changes: tasks are defined as one self-contained, independently implementable and verifiable unit of change with a clear done-condition; time-based sizing language (hour-level, story points, etc.) is explicitly forbidden; a story is a user-visible capability composed of atomic tasks; an epic is a coherent feature area composed of stories; the assistant is instructed to always create the full hierarchy when asked for a full plan and must not defer or ration artifact creation for board-tidiness reasons. No existing tests assert on prompt text content, so no test updates were needed.
